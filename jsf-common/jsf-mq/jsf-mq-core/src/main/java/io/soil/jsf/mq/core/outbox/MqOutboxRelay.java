 ...
 * <p>
 * 依赖应用开启 {@code @EnableScheduling}；扫描间隔由 {@code jsf.mq.producer.outbox.relay.interval} 控制（默认 5000ms）。
 * </p>
 *
 * @author zeno.w
 */
...
public class MqOutboxRelay {
    ...
    public MqOutboxRelay(MqOutboxStore store, MqOutbox outbox, MqProducerProperties.Outbox props) {
        this.store = store;
        this.outbox = outbox;
        this.props = props;
    }
     ...
     * @return 本轮成功发送条数
     */
    @Scheduled(fixedDelayString = "${jsf.mq.producer.outbox.relay.interval:5000}")
    public int relay() {
