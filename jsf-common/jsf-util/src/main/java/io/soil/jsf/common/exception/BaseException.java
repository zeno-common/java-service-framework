 ...
 * <p>
 * 制定异常规约：每个业务从本类派生并实现 {@link #type()} 方法，返回异常类型（{@link ExceptionType}），
 * 以便在日志和监控中快速定位异常来源。
 * <p>
 * 异常状态码：通过 {@link #code()} 返回异常状态码，用于标识具体的错误类型，
 * 格式建议为 {@code [业务]-[编码]}，例如 {@code ORDER-EXSITED}、{@code DB-SAVE-FAILED}。
 * <p>
 * 消息格式化：异常消息支持 {@link MessageFormat} 模板语法，
 * 使用 {@code {0}}、{@code {1}} 等占位符进行参数替换，例如：
 * <pre>{@code
 *   new MyException("UC-NOT-FOUND", "用户 {0} 不存在", userId)
 * }</pre>
 * <p>
